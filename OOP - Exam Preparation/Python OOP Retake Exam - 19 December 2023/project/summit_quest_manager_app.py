...
class SummitQuestManagerApp:
    ...
    def check_gear(self, climber_name: str, peak_name: str, gear: List[str]):
        ...
        if peak.get_recommended_gear() == gear:
            ...
            lack_of_items = [item for item in peak.get_recommended_gear() if item not in gear]
            sorted_lack_of_items = sorted(lack_of_items)
            climber.is_prepared = False
            return f"{climber_name} is not prepared to climb {peak_name}. Missing gear: {', '.join(sorted_lack_of_items)}."
    ...
    def get_statistics(self):
        ...
        result = []
        result.append(f"Total climbed peaks: {len(self.peaks)}\n"
