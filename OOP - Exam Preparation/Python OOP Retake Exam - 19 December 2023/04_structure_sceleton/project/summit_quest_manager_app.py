...
class SummitQuestManagerApp:
    ...
    def register_climber(self, climber_type: str, climber_name: str):
        ...
        climber = self.get_object_by_name(self.climbers, climber_name)
        if climber:
    ...
    def check_gear(self, climber_name: str, peak_name: str, gear: List[str]):
        climber = self.get_object_by_name(self.climbers, climber_name)
        peak = self.get_object_by_name(self.peaks, peak_name)

        missing_items = [item for item in peak.get_recommended_gear() if item not in gear]
        if missing_items:
            climber.is_prepared = False
            sorted_missing_items = ', '.join(sorted(missing_items))

            return f"{climber_name} is not prepared to climb {peak_name}. Missing gear: {sorted_missing_items}."
    ...
    def get_statistics(self):
        climbers_with_conquers_peaks = [c for c in self.climbers if c.conquered_peaks]
        sorted_climbers = sorted(climbers_with_conquers_peaks, key=lambda c: (-len(c.conquered_peaks), c.name))
