...
class AuctionHouseManagerApp:
    ...
    def register_artifact(self, artifact_type: str, artifact_name: str, artifact_price: float, artifact_space: int):
        ...
        artifact = self._get_object_by_name(self.artifacts, artifact_name)
        if artifact:
    ...
    def register_collector(self, collector_type: str, collector_name: str):
        ...
        collector = self._get_object_by_name(self.collectors, collector_name)
        if collector:
    ...
    def get_auction_report(self):
        ...
        count_of_sold_artifacts = sum(len(collector.purchased_artifacts) for collector in self.collectors)
        count_of_available_artifacts = len(self.artifacts)
        result.append(f"Total number of sold artifacts: {count_of_sold_artifacts}")
        result.append(f"Available artifacts for sale: {count_of_available_artifacts}")
        result.append("***")
        for collector in sorted_collectors:
            result.append(f"{collector}")
