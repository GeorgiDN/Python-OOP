...
class FactoryManager:
    ...
    def sell_products_to_store(self, store: BaseStore, *products: BaseProduct):
        ...
        for product in products:
            if (product.sub_type == "Furniture" and store.store_type == "FurnitureStore") or \
                    (product.sub_type == "Toys" and store.store_type == "ToyStore"):
    ...
    def unregister_store(self, store_name: str):
        store = self._get_object_by_name(self.stores, store_name)
        if not store:
    ...
    def statistics(self):
        ...
        for mod, count in products_data.items():
            result.append(f"{mod}: {count}")
