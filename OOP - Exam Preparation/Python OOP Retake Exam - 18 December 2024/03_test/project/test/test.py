...
class GalleryTest(unittest.TestCase):
    def setUp(self):
        self.gallery = Gallery('Test1', 'Sofia', 10.0)

    def test_init(self):
        self.assertIsInstance(self.gallery, Gallery)
        self.assertEqual(self.gallery.gallery_name, 'Test1')
        self.assertEqual(self.gallery.city, 'Sofia')
        self.assertEqual(self.gallery.area_sq_m, 10.0)
        self.assertTrue(self.gallery.open_to_public)
        self.assertEqual(self.gallery.exhibitions, {})

    def test_gallery_name_with_empty_name(self):
    ...
    def test_gallery_name_with_special_character(self):
        with self.assertRaises(ValueError) as ve:
            self.gallery.gallery_name = 'test_'
        message = "Gallery name can contain letters and digits only!"
        self.assertEqual(str(ve.exception), message)

    def test_city_start_with_empty_string(self):
        with self.assertRaises(ValueError) as ve:
            self.gallery.city = ''
    ...
    def test_add_exhibition(self):
        res = self.gallery.add_exhibition('exhibition_name1', 2000)
        message = 'Exhibition "exhibition_name1" added for the year 2000.'
        self.assertEqual(res, message)
        self.assertEqual(self.gallery.exhibitions, {'exhibition_name1': 2000})

    def test_add_exhibition_with_existing_one(self):
        self.gallery.add_exhibition('exhibition_name1', 2000)
        res = self.gallery.add_exhibition('exhibition_name1', 2000)
        message = 'Exhibition "exhibition_name1" already exists.'
        self.assertEqual(res, message)
        self.assertEqual(self.gallery.exhibitions, {'exhibition_name1': 2000})

    def test_remove_exhibition(self):
        self.gallery.add_exhibition('exhibition_name1', 2000)
        res = self.gallery.remove_exhibition('exhibition_name1')
        message = 'Exhibition "exhibition_name1" removed.'
        self.assertEqual(res, message)
        self.assertEqual(self.gallery.exhibitions, {})

    def test_remove_exhibition_with_multiply_exhibitions(self):
        self.gallery.add_exhibition('exhibition_name1', 2000)
        self.gallery.add_exhibition('exhibition_name2', 2000)
        res = self.gallery.remove_exhibition('exhibition_name1')
        message = 'Exhibition "exhibition_name1" removed.'
        self.assertEqual(res, message)
        self.assertEqual(self.gallery.exhibitions, {'exhibition_name2': 2000})

    def test_remove_exhibition_with_not_existing_one(self):
        res = self.gallery.remove_exhibition('exhibition_name1')
        message = 'Exhibition "exhibition_name1" not found.'
        self.assertEqual(res, message)

    def test_list_exhibitions_with_open_to_public(self):
        self.gallery.add_exhibition('exhibition_name1', 2000)
        self.gallery.add_exhibition('exhibition_name2', 2000)
        res = self.gallery.list_exhibitions()
        expected = 'exhibition_name1: 2000\nexhibition_name2: 2000'
    ...
    def test_list_exhibitions_with_not_open_to_public(self):
        ...
        self.gallery.add_exhibition('exhibition_name2', 2000)
        res = self.gallery.list_exhibitions()
        expected = f'Gallery {self.gallery.gallery_name} is currently closed for public! Check for updates later on.'
        self.assertEqual(res, expected)
