class PizzaDelivery:
    ...
    def add_extra(self, ingredient: str, quantity: int, price_per_quantity: float):
        if self.ordered:
            return self._already_prepared_message()
    ...
    def make_order(self):
        if self.ordered:
            return self._already_prepared_message()
        self.ordered = True
        ingredients_data = [f"{ing}: {qty}" for ing, qty in self.ingredients.items()]
        return f"You've ordered pizza {self.name} prepared with {', '.join(ingredients_data)} and the price will be {self.price}lv."

    def _already_prepared_message(self):
